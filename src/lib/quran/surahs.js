...
export const QURAN_SURAH_OPTIONS = QURAN_SURAHS.map((surah) => ({
  value: surah.name,
  label: surah.name,
  number: surah.number,
  ayahs: surah.ayahs,
...
export function getSurahByName(name) {
  return QURAN_SURAHS.find((surah) => surah.name === name) || null;
}

export function getSurahAyahCountByName(name) {
  return getSurahByName(name)?.ayahs || 0;
}

export function getSurahNumberByName(name) {
  return getSurahByName(name)?.number || 0;
}
